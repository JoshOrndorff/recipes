...
// these type assignments work but are just placeholders
impl execution_schedule::Trait for Runtime {
    type Event = Event;
    type ExecutionFrequency = ClearFrequency;
    type SignalQuota = MaxAddend;
    type TaskLimit = ClearFrequency;
}

impl smpl_treasury::Trait for Runtime {
    type Event = Event;
    type Currency = balances::Module<Runtime>;
    type Tax = MaxAddend;
