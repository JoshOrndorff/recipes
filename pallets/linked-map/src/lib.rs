...
decl_storage! {
	trait Store for Module<T: Trait> as List {
		TheList get(fn the_list): map hasher(blake2_256) u32 => T::AccountId;
		TheCounter get(fn the_counter): u32;

		LinkedList get(fn linked_list): linked_map hasher(blake2_256) u32 => T::AccountId;
...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		fn remove_member_unbounded(origin, index: u32) -> DispatchResult {
			...
			// verify existence
			ensure!(<TheList<T>>::contains_key(index), "an element doesn't exist at this index");
		...
		fn remove_member_linked(origin, index: u32) -> DispatchResult {
			...
			ensure!(<LinkedList<T>>::contains_key(index), "A member does not exist at this index");
