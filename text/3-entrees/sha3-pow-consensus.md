...
We begin as before by defining a struct that will implement the `PowAlgorithm` trait. Unlike before, this struct must hold a reference to the [`Client`](https://substrate.dev/rustdocs/v2.0.0-alpha.8/sc_client/struct.Client.html) so it can call the appropriate runtime APIs.
